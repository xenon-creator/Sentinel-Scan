...
async def get_indicator_details(indicator_id: str, db: AsyncIOMotorDatabase=Depends(get_db)):
    ...
    if finding.get('severity') in ['critical', 'high']:
        ...
        classification = 'Suspicious'
    return {'finding_id': finding.get('finding_id'), 'indicator': finding.get('affected_asset'), 'title': finding.get('title'), 'description': finding.get('description'), 'threat_score': (finding.get('risk_score') or 0) * 10, 'classification': classification, 'severity': finding.get('severity'), 'timestamp': finding.get('discovered_at'), 'port': finding.get('port'), 'protocol': finding.get('protocol'), 'scan_status': scan.get('status') if scan else 'Unknown', 'raw_data': {'finding': {k: v for k, v in finding.items() if k != '_id'}, 'scan': {k: v for k, v in scan.items() if k != '_id'} if scan else None}}
