...
async def get_finding(finding_id: str, db: AsyncIOMotorDatabase = Depends(get_db)):
    ...
    return FindingResponse(
        ...
        status=FindingStatus(finding.get("status", "open")),
...
async def update_finding(
    ...
    if update_request.notes:
        await db.findings.update_one(
            {"finding_id": finding_id}, {"$push": {"notes": update_request.notes}}
        )
    ...
    return FindingResponse(
        ...
        severity=Severity(updated_finding["severity"]),
        cvss_score=updated_finding.get("cvss_score"),
        cve_id=updated_finding.get("cve_id"),
        affected_asset=updated_finding["affected_asset"],
        status=FindingStatus(updated_finding.get("status", "open")),
        discovered_at=updated_finding["discovered_at"],
        updated_at=updated_finding.get("updated_at"),
    )
