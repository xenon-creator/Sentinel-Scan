...
from backend.models.asset import AssetDocument, AssetRequest, AssetResponse
...
async def register_asset(
    asset_request: AssetRequest, db: AsyncIOMotorDatabase = Depends(get_db)
):
    asset_doc = AssetDocument(
        hostname=asset_request.hostname,
        ip_address=asset_request.ip_address,
        asset_type=asset_request.asset_type.value,
        criticality=asset_request.criticality.value,
    ...
    if asset_doc.ip_address:
        ...
        if existing:
            raise HTTPException(
                status_code=409,
                detail=f"Asset with IP {asset_doc.ip_address} already exists",
            )
    await db.assets.insert_one(asset_doc.model_dump())
    return AssetResponse(
        asset_id=asset_doc.asset_id,
        hostname=asset_doc.hostname,
        ip_address=asset_doc.ip_address,
        asset_type=asset_request.asset_type,
        criticality=asset_request.criticality,
        tags=asset_doc.tags,
...
async def list_assets(
    limit: int = 100, offset: int = 0, db: AsyncIOMotorDatabase = Depends(get_db)
):
