...
class PluginManager:
    ...
    async def execute_scan(
        self, plugin_name: str, target: str, options: Optional[Dict] = None
    ) -> ScanResult:
        if plugin_name not in self.plugins:
            return ScanResult(
        ...
        if not plugin.initialized:
            ...
            if not init_success:
                return ScanResult(
                    ...
                    status="failed",
                    findings=[],
                    error_message=f"Failed to initialize plugin '{plugin_name}'",
                )
        if not plugin.validate_target(target):
            return ScanResult(
                ...
                status="failed",
                findings=[],
                error_message=f"Invalid target for plugin '{plugin_name}'",
            )
        try:
