...
class NmapScanner(ScannerPlugin):
    ...
    async def scan(self, target: str, options: Optional[Dict]=None) -> ScanResult:
        ...
        try:
            await asyncio.to_thread(self.nm.scan, hosts=target, ports=port_range, arguments=nmap_args)
            findings = self.parse_output(self.nm)
            scan_duration = (datetime.utcnow() - start_time).total_seconds()
            return ScanResult(scanner_name=self.name, target=target, status='success', findings=findings, raw_output=str(self.nm.csv()), scan_duration=scan_duration, metadata={'nmap_version': self.nm.nmap_version(), 'command_line': self.nm.command_line(), 'hosts_scanned': len(self.nm.all_hosts())})
        except Exception as e:
            scan_duration = (datetime.utcnow() - start_time).total_seconds()
            return ScanResult(scanner_name=self.name, target=target, status='failed', findings=[], error_message=str(e), scan_duration=scan_duration)
            ...
            for proto in host_info.all_protocols():
                ...
                for port in ports:
                    ...
                    if port_info['state'] == 'open':
                        finding = {'title': f'Open Port: {port}/{proto}', 'description': f'Port {port}/{proto} is open on {host}', 'severity': self._determine_severity(port, port_info), 'affected_asset': host, 'port': port, 'protocol': proto, 'service': port_info.get('name', 'unknown'), 'affected_component': f"{port_info.get('name', 'unknown')} {port_info.get('version', '')}".strip(), 'metadata': {'state': port_info['state'], 'reason': port_info.get('reason', ''), 'product': port_info.get('product', ''), 'version': port_info.get('version', ''), 'extrainfo': port_info.get('extrainfo', ''), 'os': os_info}}
